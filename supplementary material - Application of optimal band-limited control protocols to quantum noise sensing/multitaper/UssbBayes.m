%% UssbBayes
%returns average of DPSS filters (for each k)  in each segment
...
%each entry is the average value of the filter corresponding to the DPSS of
%order k in the segment

function out=UssbBayes(N,W,w0,delT,K,oms)
...
WF=cell2mat(arrayfun(@(k)(trapz(transpose(oms_i),4*(abs(DFTc*vssb(:,k)).^2).*...
    (sin(delT*transpose(oms_i)/2).^2)./((oms(i+1)-oms(i))*(transpose(oms_i).^2+.01)))),1:nk,...
    'UniformOutput',false));
seg_avs(i,:)=WF;
end
